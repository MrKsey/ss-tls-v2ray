...
status() {
    STATUS=$(netstat -tulpn | grep "sslocal" | grep ":$SS_LOCAL_PORT"); [ -z "$STATUS" ] && \
    ...
    printf "${GREEN}Client ShadowSocks is running:${NC}\n"; netstat -tulpn | grep "sslocal" | grep ":$SS_LOCAL_PORT"; echo; printf "Remote server: ${GREEN}$SS_SERVER_ADDR:$SS_SERVER_PORT${NC}\n"
}
