...
status() {
    STATUS=$(netstat -tulpn | grep "sslocal" | grep ":$SIMPLE_TLS_LOCAL_PORT"); [ -z "$STATUS" ] && \
    ...
    printf "${GREEN}Client SS+Simple-TLS is running:${NC}\n"; netstat -tulpn | grep "sslocal" | grep ":$SIMPLE_TLS_LOCAL_PORT"; echo; printf "Remote server: ${GREEN}$SS_SERVER_ADDR:$SIMPLE_TLS_SERVER_PORT${NC}\n"
}
