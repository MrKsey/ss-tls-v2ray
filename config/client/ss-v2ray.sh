...
status() {
    STATUS=$(netstat -tulpn | grep "sslocal" | grep ":$V2RAY_LOCAL_PORT"); [ -z "$STATUS" ] && \
    ...
    printf "${GREEN}Client SS+V2RAY is running:${NC}\n"; netstat -tulpn | grep "sslocal" | grep ":$V2RAY_LOCAL_PORT"; echo; printf "Remote server: ${GREEN}$SS_SERVER_ADDR:$V2RAY_SERVER_PORT${NC}\n"
}
