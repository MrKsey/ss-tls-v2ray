...
if [ "$MODE" = "server" ]; then
	...
	sed -i "/^SS_SERVER_PORT=/{h;s/=.*/=${SS_SERVER_PORT}/};\${x;/^$/{s//SS_SERVER_PORT=${SS_SERVER_PORT}/;H};x}" $CONFIG_PATH/config.ini
	...
	sed -i "/^SIMPLE_TLS_SERVER_PORT=/{h;s/=.*/=${SIMPLE_TLS_SERVER_PORT}/};\${x;/^$/{s//SIMPLE_TLS_SERVER_PORT=${SIMPLE_TLS_SERVER_PORT}/;H};x}" $CONFIG_PATH/config.ini
	...
	sed -i "/^V2RAY_SERVER_PORT=/{h;s/=.*/=${V2RAY_SERVER_PORT}/};\${x;/^$/{s//V2RAY_SERVER_PORT=${V2RAY_SERVER_PORT}/;H};x}" $CONFIG_PATH/config.ini
    ...
    # CLOACK_KEY
    if [ -s "$CONFIG_PATH/server/cloack_keys.txt" ]; then
        export CLOACK_PRIVATE_KEY=$(grep -E -o "^CLOACK_PRIVATE_KEY=.+" "$CONFIG_PATH/server/cloack_keys.txt" | sed 's/CLOACK_PRIVATE_KEY=//')
        export CLOACK_PUBLIC_KEY=$(grep -E -o "^CLOACK_PUBLIC_KEY=.+" "$CONFIG_PATH/server/cloack_keys.txt" | sed 's/CLOACK_PUBLIC_KEY=//') 
    else
        export CLOACK_PRIVATE_KEY=""
        export CLOACK_PUBLIC_KEY=""
	fi
    ...
    if [ -z "$CLOACK_PRIVATE_KEY" ] || [ -z "$CLOACK_PUBLIC_KEY" ]; then
        ...
        echo "CLOACK_PUBLIC_KEY=$CLOACK_PUBLIC_KEY" > $CONFIG_PATH/server/cloack_keys.txt
        echo "CLOACK_PRIVATE_KEY=$CLOACK_PRIVATE_KEY" >> $CONFIG_PATH/server/cloack_keys.txt
    fi
    ...
    jq --arg SS_SERVER_PORT "$SS_SERVER_PORT" \
       ...
       .AdminUID = $CLOACK_ADMIN_UID' $CONFIG_PATH/server/ckserver-template.json > $CONFIG_PATH/server/ckserver.json
